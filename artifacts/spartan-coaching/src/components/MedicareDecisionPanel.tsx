...
export function MedicareDecisionPanel() {
  ...
  return <div className="space-y-5">
    <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.09] via-card to-card p-5 sm:p-7">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Decision Room</p>
        ...
        <div className="rounded-2xl border border-border bg-background/75 p-4 text-xs leading-5 text-muted-foreground">
    ...
    <Card className="space-y-4 p-5 sm:p-6">
      <div className="grid gap-4 md:grid-cols-[14rem_minmax(0,1fr)]">
        ...
        <div className="space-y-2"><Label htmlFor="decision-goal">Decision you need to make</Label><Textarea id="decision-goal" value={goal} onChange={(event) => setGoal(event.target.value)} rows={3} placeholder="Example: Decide whether this account deserves focused field development this quarter." /></div>
      </div>
    ...
    {brief ? <Card className="space-y-6 border-primary/30 p-5 sm:p-7" data-testid="medicare-decision-brief">
      ...
      <DecisionSection title="Recommended move"><p className="rounded-2xl border border-primary/20 bg-primary/[0.07] p-4 text-base font-bold leading-7 text-foreground">{brief.recommendedMove}</p></DecisionSection>
      ...
      <DecisionSection title="Execution plan"><div className="space-y-3">{brief.nextActions.map((item, index) => <article key={item.timing} className="grid gap-3 rounded-2xl border border-border p-4 sm:grid-cols-[2rem_8rem_minmax(0,1fr)]"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary text-xs font-black text-primary-foreground">{index + 1}</span><p className="text-xs font-black uppercase tracking-wide text-primary">{item.timing}</p><div><p className="text-sm font-semibold text-foreground">{item.action}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Success: {item.successSignal}</p></div></article>)}</div></DecisionSection>
...
function DecisionSection({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="space-y-3"><h4 className="text-[10px] font-black uppercase tracking-[0.18em] text-primary"><AccentText>{title}</AccentText></h4>{children}</section>;
}
