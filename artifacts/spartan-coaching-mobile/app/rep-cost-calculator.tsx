...
export default function RepCostCalculatorScreen() {
  ...
  const [values, setValues] = useState<Record<keyof RepCostInputs, string>>({ baseSalary: "90000", benefitsLoad: "42", annualMileage: "5400", otherFixedCosts: "15484", callsPerDay: "12", workingDaysPerMonth: "20", callsPerReferral: "8", conversionRate: "70", netContributionPerAdmission: "5000" });
  const [tiers, setTiers] = useState<CommissionTier[]>(INITIAL_TIERS);
  const input = useMemo<RepCostInputs>(() => ({
    baseSalary: positive(values.baseSalary), benefitsLoad: positive(values.benefitsLoad), annualMileage: positive(values.annualMileage), otherFixedCosts: positive(values.otherFixedCosts), callsPerDay: positive(values.callsPerDay), workingDaysPerMonth: positive(values.workingDaysPerMonth), callsPerReferral: Math.max(0.01, positive(values.callsPerReferral)), conversionRate: Math.min(100, positive(values.conversionRate)), netContributionPerAdmission: positive(values.netContributionPerAdmission),
  }), [values]);
  ...
  const breakEven = (value: number | null) => value == null ? "Enter contribution" : String(value);
  const report = ["Spartan Coaching Rep Economics", `Net contribution per admission: ${money(input.netContributionPerAdmission)}`, `Monthly fixed rep cost: ${money(result.monthlyFixedCost)}`, `Monthly break-even admissions: ${breakEven(result.monthlyBreakEvenAdmissions)}`, `Annual break-even admissions: ${breakEven(result.annualBreakEvenAdmissions)}`, `Monthly contribution after rep cost: ${money(result.monthlyContributionAfterRepCost)}`, `Annual contribution after rep cost: ${money(result.annualContributionAfterRepCost)}`, `Total annual rep cost: ${money(result.totalRepCost)}`, `Annual commission: ${money(result.annualCommission)}`, `Cost per call: ${money(result.costPerCall)}`, `Cost per referral: ${money(result.costPerReferral)}`, `Blended cost per admit: ${money(result.blendedCostPerAdmit)}`, `Monthly admissions: ${result.monthlyAdmissions.toFixed(1)}`, `Annual conversion leakage: ${money(result.annualConversionLoss)}`, `Active tier: ${result.activeTier?.min ?? 0} to ${result.activeTier?.max === 999 ? "unlimited" : result.activeTier?.max ?? 0} admits at ${money(result.activeTier?.rate ?? 0)} per admit`, "Planning estimate only. Validate compensation and cost assumptions with finance and human resources."].join("\n");
  ...
  return <>
    ...
    <ScrollView style={{ backgroundColor: colors.background }} contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, paddingBottom: insets.bottom + 40 }} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" testID="rep-cost-calculator-workspace">
      ...
      <CalculatorSection eyebrow="01 · LOADED COST" title="Build the annual cost base">
        ...
        <CalculatorField label="Net contribution per admission" value={values.netContributionPerAdmission} onChangeText={set("netContributionPerAdmission")} prefix="$" hint="Use finance-approved revenue retained after patient-care costs and before sales commission, not gross reimbursement." />
      </CalculatorSection>
      ...
      <CalculatorSection eyebrow="04 · DECISION VIEW" title="Unit economics and leakage">
        <MetricGrid metrics={[
          { label: "Monthly break-even admissions", value: breakEven(result.monthlyBreakEvenAdmissions), detail: "Whole admissions required to cover monthly rep cost", emphasis: true },
          { label: "Annual break-even admissions", value: breakEven(result.annualBreakEvenAdmissions), detail: "Whole admissions required to cover annual rep cost", emphasis: true },
          { label: "Monthly contribution after rep cost", value: money(result.monthlyContributionAfterRepCost), detail: result.monthlyContributionAfterRepCost >= 0 ? "Above break-even" : "Below break-even", emphasis: true },
          { label: "Annual contribution after rep cost", value: money(result.annualContributionAfterRepCost), detail: result.annualContributionAfterRepCost >= 0 ? "Above break-even" : "Below break-even", emphasis: true },
          { label: "Blended cost per admit", value: money(result.blendedCostPerAdmit), detail: "Fixed cost plus commission", emphasis: true }, { label: "Total annual rep cost", value: money(result.totalRepCost), detail: "Loaded annual cost" }, { label: "Cost per referral", value: money(result.costPerReferral), detail: `${result.monthlyReferrals.toFixed(1)} referrals monthly` }, { label: "Annual conversion leakage", value: money(result.annualConversionLoss), detail: `${result.annualLostAdmissions.toFixed(1)} unconverted referrals` }]} />
      ...
      <DecisionBrief title="Executive readout" interpretation={`The modeled rep costs ${money(result.totalRepCost)} annually and produces ${result.monthlyAdmissions.toFixed(1)} monthly admissions. At ${money(input.netContributionPerAdmission)} net contribution per admission, the rep needs ${breakEven(result.monthlyBreakEvenAdmissions)} admissions per month and ${breakEven(result.annualBreakEvenAdmissions)} per year to break even. Modeled annual contribution after rep cost is ${money(result.annualContributionAfterRepCost)}.`} actions={["Confirm loaded cost and commission assumptions with finance and human resources.", "Coach the conversion gap before adding more call volume.", "Compare the same model across representatives using identical definitions."]} caution="This model does not establish compensation policy or employment terms. Review any compensation change with finance, human resources, legal, and compliance." />
      <CalculatorReportActions kind="rep-cost" title="Rep Economics" summary={`${breakEven(result.monthlyBreakEvenAdmissions)} monthly admissions and ${breakEven(result.annualBreakEvenAdmissions)} annual admissions required to break even.`} report={report} />
    </ScrollView>
  </>;
}
