...
/**
 * Scene 6 — Brand Close.
 * The Spartan stamp slams into center frame with a red shock pulse
 * and frame shake, then the lockup rises beneath it and holds.
 */
const Scene6: React.FC<{ duration: number }> = () => {
  ...
  return (
    ...
    >
      {/* Faint warm continuity in the background */}
      <motion.div
        className="absolute inset-0 z-0"
        initial={{ opacity: 0, scale: 1.08 }}
        animate={{ opacity: 0.25, scale: 1 }}
        exit={{ opacity: 0 }}
        transition={{ opacity: { duration: 1.8, ease: 'easeOut' }, scale: { duration: 8, ease: 'linear' } }}
      >
        <img
      ...
      >
        {/* The stamp — slams in from above, oversized and dominant */}
        <motion.div
        ...
        <div className="flex flex-col items-center text-center mt-[3vh] px-[5vw] w-full">
          <div className="overflow-hidden w-full">
            <motion.h2
              className="font-display font-bold text-[5.4vw] leading-[1.15] text-[var(--color-brand-white)] text-shadow-heavy pb-[0.5vh]"
              initial={{ y: '110%' }}
              animate={{ y: 0 }}
          ...
          <motion.p
            className="font-display italic text-[2.4vw] leading-[1.4] text-[var(--color-brand-warm)] text-shadow-subtle mt-[1.5vh]"
          ...
          {/* Red rule — echoes the strike from the indictment */}
          <motion.div
            className="h-[0.5vh] w-[28vw] bg-[var(--color-brand-red)] mt-[2.5vh] origin-left"
            initial={{ scaleX: 0, opacity: 0 }}
            animate={{ scaleX: 1, opacity: 1 }}
            transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: 3.1 }}
          />
        </div>
      </motion.div>
    </motion.div>
