...
describe.runIf(integrationEnabled)("AI tool PostgreSQL tenant isolation", () => {
  ...
  afterAll(async () => {
    ...
    await db.delete(clinicalCases).where(inArray(clinicalCases.organizationId, orgIds));
    await db.delete(aiToolRuns).where(inArray(aiToolRuns.organizationId, orgIds));
    await db
    ...
    await db
      ...
      .where(inArray(clientSessions.memberId, [firstMemberId, secondMemberId]));
    await db
  ...
  it("never exposes another organization's cases, documents, or deletion path", async () => {
    const cases = await request(app)
      .get("/api/clinical/cases")
      .set("Authorization", `Bearer ${token}`);
    expect(cases.status).toBe(200);
    expect(cases.body.cases).toEqual([]);
