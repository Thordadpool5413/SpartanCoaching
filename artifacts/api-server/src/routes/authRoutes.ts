...
export function registerAuthRoutes(app: Express): void {
  ...
  /**
   * One-time admin tool: set a known password on the Apple reviewer account and permanently
   * activate its org. Requires a platform_admin session. Accepts the password from the
   * request body so nothing sensitive is hardcoded here.
   *
   * Usage (after publishing):
   *   curl -X POST https://<host>/api/admin/activate-reviewer \
   *     -H "Content-Type: application/json" \
   *     -H "Authorization: Bearer <admin-token>" \
   *     -d '{"password":"<reviewer-password>"}'
   *
   * DELETE this endpoint from the codebase once the reviewer account is confirmed working
   * in production (see follow-up task).
   */
  app.post("/api/admin/activate-reviewer", requireAdmin, async (req, res) => {
    try {
      const { password } = req.body ?? {};
      if (!password || typeof password !== "string" || password.length < 8) {
        return res.status(400).json({ error: "password (min 8 chars) is required in request body" });
      }
      ...
      if (!member) {
        return res.status(404).json({ error: "Reviewer account not found. Run seed script first." });
      }
      ...
      await db
        ...
        .set({
          passwordHash,
          status: "active",
          termsAcceptedAt: member.termsAcceptedAt ?? new Date(),
        })
        .where(eq(clientMembers.id, member.id));
      ...
      await logEvent("admin_action", member.id, { action: "reviewer_activated" });

      return res.json({
        ok: true,
        message: "Reviewer account activated.",
        email: REVIEWER_EMAIL,
      });
    } catch (err) {
      console.error("activate-reviewer error:", err);
      return res.status(500).json({ error: "Failed to activate reviewer account" });
    }
  });
