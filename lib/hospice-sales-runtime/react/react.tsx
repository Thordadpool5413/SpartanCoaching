...
const dayBounds = (date: string) => { const [year,month,day]=date.split("-").map(Number);const start=new Date(year,month-1,day);const end=new Date(year,month-1,day+1);return [start.toISOString(),end.toISOString()] as const; };
const time = (iso?: string) => iso ? new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit" }).format(new Date(iso)) : "—";

export function SalesWorkflowPanel({ api, actor, initialView = "today", theme = {} }: SalesWorkflowPanelProps) {
  const [view, setView] = useState(initialView); const [date, setDate] = useState(new Date().toISOString().slice(0,10)); const [data, setData] = useState<TodayData>({ calls: [],plans:[], actions: [], syncJobs: [] }); const [accounts, setAccounts] = useState<Account[]>([]); const [state, setState] = useState<"loading"|"ready"|"offline"|"error">("loading"); const [message, setMessage] = useState(""); const [dialog, setDialog] = useState<"schedule"|"complete"|"next"|"practice"|"import"|null>(null); const [selectedCall, setSelectedCall] = useState<SalesCall>(); const [selectedAction,setSelectedAction]=useState<NextAction>();const[selectedSession,setSelectedSession]=useState<RoleplaySession>();
  const load = async () => { setState("loading"); try { const [from,to] = dayBounds(date); const [today, accountData] = await Promise.all([api.today(from,to), api.accounts()]); setData(today); setAccounts(accountData.accounts); setState("ready"); } catch (error) { setState(typeof navigator !== "undefined" && !navigator.onLine ? "offline" : "error"); setMessage(error instanceof Error ? error.message : "Unable to load your workspace"); } };
  useEffect(() => { void load(); }, [date]); const overdue = useMemo(() => data.actions.filter((item) => item.dueAt && item.dueAt < new Date().toISOString()), [data.actions]); const warningJobs = data.syncJobs.filter((item) => ["retryable","conflicted","dead_lettered"].includes(String(item.status)));
  return <main className="hsw" style={theme as React.CSSProperties}>
    <a className="hsw-skip" href="#hsw-main">Skip to workspace</a>
    <header className="hsw-top"><div><span className="hsw-kicker">Field rhythm</span><h1>Sales daybook</h1></div><nav aria-label="Workspace"><Tab active={view==="today"} onClick={()=>setView("today")}>Today</Tab><Tab active={view==="accounts"} onClick={()=>setView("accounts")}>Accounts</Tab>{actor.role==="manager"&&<Tab active={view==="manager"} onClick={()=>setView("manager")}>Team</Tab>}<Tab active={view==="integrations"} onClick={()=>setView("integrations")}>Connections</Tab></nav><button className="hsw-primary" onClick={()=>setDialog("schedule")}>Schedule call</button></header>
    <section id="hsw-main" className="hsw-main" tabIndex={-1}>
      {state==="loading"&&<Status title="Preparing your day" body="Bringing calls, next steps, and account context together." busy/>}
      {state==="offline"&&<Status title="You’re offline" body="Your saved view remains available. Reconnect to sync changes." action={<button onClick={load}>Try again</button>}/>}
      {state==="error"&&<Status title="Workspace unavailable" body={message} action={<button onClick={load}>Retry</button>}/>}
      {state==="ready"&&view==="today"&&<TodayView date={date} setDate={setDate} data={data} overdue={overdue} warnings={warningJobs} onBuild={async plan=>{setMessage("Building plan…");try{await api.buildPlan(plan.id,{expectedVersion:plan.version});setMessage("Pre-call plan ready");await load()}catch(error){setMessage(error instanceof Error?error.message:"Could not build plan")}}} onPractice={async plan=>{setMessage("Starting roleplay…");try{setSelectedSession(await api.startRoleplay(plan.id,{expectedVersion:plan.version}));setDialog("practice");setMessage("")}catch(error){setMessage(error instanceof Error?error.message:"Could not start roleplay")}}} onComplete={(call)=>{setSelectedCall(call);setDialog("complete")}} onScheduleNext={(action)=>{setSelectedAction(action);setDialog("next")}} onDraftEmail={async action=>{try{const draft=await api.generateEmailDraft(action.id,{expectedVersion:action.version});setMessage(`Email draft ready: ${draft.subject}`);await load()}catch(error){setMessage(error instanceof Error?error.message:"Could not create email draft")}}}/>}
      {state==="ready"&&view==="accounts"&&<AccountView accounts={accounts} onImport={()=>setDialog("import")}/>}
      {state==="ready"&&view==="manager"&&<ManagerView data={data}/>}
    ...
    <div className="hsw-live" aria-live="polite">{message}</div>
  </main>;
}
function Tab({active,onClick,children}:{active:boolean;onClick():void;children:ReactNode}){return <button className={active?"is-active":""} aria-current={active?"page":undefined} onClick={onClick}>{children}</button>}
function Status({title,body,busy,action}:{title:string;body:string;busy?:boolean;action?:ReactNode}){return <div className="hsw-status" role={busy?"status":"alert"}><span className={busy?"hsw-pulse":"hsw-mark"}/><h2>{title}</h2><p>{body}</p>{action}</div>}
function TodayView({date,setDate,data,overdue,warnings,onBuild,onPractice,onComplete,onScheduleNext,onDraftEmail}:{date:string;setDate(value:string):void;data:TodayData;overdue:NextAction[];warnings:SyncJob[];onBuild(plan:CallPlan):void;onPractice(plan:CallPlan):void;onComplete(call:SalesCall):void;onScheduleNext(action:NextAction):void;onDraftEmail(action:NextAction):void}){return <><section className="hsw-hero"><div><label htmlFor="work-date">Working date</label><input id="work-date" type="date" value={date} onChange={(event)=>setDate(event.target.value)}/><h2>{data.calls.length ? `${data.calls.length} conversations on deck` : "A clear field day"}</h2><p>Prepare deliberately. Capture what changed. Leave every account with a visible next move.</p></div><div className="hsw-score"><strong>{data.actions.length}</strong><span>open next steps</span></div></section><div className="hsw-columns"><section className="hsw-card hsw-agenda"><header><span>Run of day</span><strong>{new Intl.DateTimeFormat(undefined,{weekday:"long",month:"long",day:"numeric"}).format(new Date(`${date}T12:00:00`))}</strong></header>{data.calls.length===0?<Empty title="No calls scheduled" body="Use Schedule call to create the first plan for this day."/>:<ol>{data.calls.map((call,index)=>{const plan=data.plans.find(item=>item.callId===call.id);const callStatus=String(call.status);return <li key={call.id}><div className="hsw-time">{time(String(call.schedule.startsAt))}</div><div><span className="hsw-index">{String(index+1).padStart(2,"0")}</span><h3>{call.purpose}</h3><p>{String(call.schedule.location||"Location not set")} · {String(call.schedule.durationMinutes)} min</p><span className={`hsw-badge hsw-${callStatus}`}>{callStatus.replace("_"," ")}</span>{plan&&<span className="hsw-plan-state">Plan: {String(plan.status)}</span>}</div><div className="hsw-row-actions">{plan?.status==="draft"&&<button onClick={()=>onBuild(plan)}>Prepare</button>}{plan?.status==="ready"&&<button onClick={()=>onPractice(plan)}>Practice</button>}{!["completed","canceled","no_show"].includes(callStatus)&&<button onClick={()=>onComplete(call)}>Complete</button>}</div></li>})}</ol>}</section><aside className="hsw-stack"><Metric label="Overdue" value={overdue.length} tone={overdue.length?"alert":"calm"}/><Metric label="Sync warnings" value={warnings.length} tone={warnings.length?"alert":"calm"}/><section className="hsw-card"><header><span>Next moves</span></header>{data.actions.length===0?<p className="hsw-muted">No open actions.</p>:<ul className="hsw-action-list">{data.actions.slice(0,5).map(item=><li key={item.id}><span>{item.type.replace("_"," ")}</span><strong>{item.title}</strong>{item.dueAt&&<small>{time(item.dueAt)}</small>}{item.type==="next_call"&&item.status==="accepted"&&<button onClick={()=>onScheduleNext(item)}>Schedule</button>}{item.type==="email"&&item.status==="accepted"&&<button onClick={()=>onDraftEmail(item)}>Create draft</button>}</li>)}</ul>}</section></aside></div></>}
function Metric({label,value,tone}:{label:string;value:number;tone:string}){return <div className={`hsw-metric hsw-${tone}`}><span>{label}</span><strong>{value}</strong></div>}
function Empty({title,body}:{title:string;body:string}){return <div className="hsw-empty"><span>＋</span><h3>{title}</h3><p>{body}</p></div>}
function AccountView({accounts,onImport}:{accounts:Account[];onImport():void}){return <section className="hsw-page"><header className="hsw-page-head"><div><span className="hsw-kicker">Relationship ledger</span><h2>Accounts</h2><p>One history for preparation, calls, coaching, and follow-through.</p></div><button onClick={onImport}>Import CSV</button></header>{accounts.length===0?<Empty title="No accounts yet" body="Import a CSV or schedule a call to establish the first account."/>:<div className="hsw-account-grid">{accounts.map(account=><article className="hsw-account" key={account.id}><span>{account.accountType||"Healthcare account"}</span><h3>{account.name}</h3><p>{account.address||"Address not added"}</p><div><small>Owner</small><strong>{account.ownerUserId.slice(0,8)}</strong></div></article>)}</div>}</section>}
function ManagerView({data}:{data:TodayData}){const completed=data.calls.filter(c=>c.status==="completed").length;return <section className="hsw-page"><header className="hsw-page-head"><div><span className="hsw-kicker">Coaching lens</span><h2>Team follow-through</h2><p>Approved summaries and commitments only. Raw transcripts remain private.</p></div></header><div className="hsw-manager"><Metric label="Calls scheduled" value={data.calls.length} tone="calm"/><Metric label="Completed" value={completed} tone="calm"/><Metric label="Open actions" value={data.actions.length} tone={data.actions.length?"alert":"calm"}/></div><div className="hsw-privacy"><strong>Transcript boundary active</strong><p>Managers can review approved coaching artifacts, not raw call transcripts.</p></div></section>}
function IntegrationsView({warnings,onCalendar,onCsv}:{warnings:SyncJob[];onCalendar(provider:"google"|"outlook"):void;onCsv():void}){return <section className="hsw-page"><header className="hsw-page-head"><div><span className="hsw-kicker">Connected fieldwork</span><h2>Connections</h2><p>Website-approved fields remain authoritative. Provider conflicts never overwrite silently.</p></div></header><div className="hsw-account-grid"><Connection name="Google Calendar" detail="Two-way events, reminders, recurrence" action="Connect Google" onAction={()=>onCalendar("google")}/><Connection name="Microsoft Outlook" detail="Graph calendar delta synchronization" action="Connect Outlook" onAction={()=>onCalendar("outlook")}/><Connection name="CRM import" detail="Production CSV import now · named live connectors require a configured adapter" action="Import CSV" onAction={onCsv}/></div>{warnings.length>0&&<div className="hsw-privacy hsw-warning"><strong>{warnings.length} sync jobs need attention</strong><p>Completed work is safe. Retry or resolve conflicts from integration administration.</p></div>}</section>}
function Connection({name,detail,action,onAction}:{name:string;detail:string;action:string;onAction():void}){return <article className="hsw-account"><span>Available adapter</span><h3>{name}</h3><p>{detail}</p><button onClick={onAction}>{action}</button></article>}
function Dialog({title,onClose,children}:{title:string;onClose():void;children:ReactNode}){return <div className="hsw-modal" role="presentation" onMouseDown={(e)=>e.target===e.currentTarget&&onClose()}><section role="dialog" aria-modal="true" aria-labelledby="dialog-title"><header><h2 id="dialog-title">{title}</h2><button aria-label="Close dialog" onClick={onClose}>×</button></header>{children}</section></div>}
